test_that("<ds_as_unique>", {
  ...
  ## 保留排列在前面的行
  d <- rbind(all |> slice(1:3),
             all |> slice(1:2) |> mutate(mpg = 1000))
  (d |> ds_as_unique("id"))$mpg[[1]] |>
    testthat::expect_equal(all$mpg[[1]])
  
  d <- rbind(all |> slice(1:3) |> mutate(mpg = 1000),
             all |> slice(1:2))
  (d |> ds_as_unique("id"))$mpg[[1]] |>
    testthat::expect_equal(1000)
